...
async def create_forecast(
    ...
    try:
        ...
        with pd.ExcelWriter(
            ...
            # ========== CONDITIONAL FORMATTING FOR ACCURACY COLUMNS ==========
            # Apply color scale that compares accuracy ACROSS models for each row
            # This highlights the best-performing model for each week
            
            # Find all model accuracy columns
            accuracy_col_letters = []
            accuracy_col_names = []
            
            for col_name in df_out.columns:
                if col_name.endswith('_acc'):
                    col_idx = df_out.columns.get_loc(col_name)
                    col_letter = _col_number_to_letter(col_idx)
                    accuracy_col_letters.append(col_letter)
                    accuracy_col_names.append(col_name)
            
            print(f"\n===== Applying cross-model conditional formatting =====")
            print(f"Accuracy columns: {accuracy_col_names}")
            print(f"Column letters: {accuracy_col_letters}")
            
            if len(accuracy_col_letters) >= 2:
                # Build the range string for MAX formula (e.g., "E2:G2,I2:K2")
                # This will be the range of all accuracy columns in each row
                
                # Apply conditional formatting to each accuracy column
                for col_letter in accuracy_col_letters:
                    # For each row, we'll have multiple conditional format rules
                    
                    # Rule 1: If this cell is the MAX in its row → Dark Green
                    for row in range(2, n_rows + 2):  # Start at row 2 (after header)
                        # Build list of all accuracy cells in this row
                        row_cells = ','.join([f'{letter}{row}' for letter in accuracy_col_letters])
                        
                        # Dark green for maximum value in row
                        ...
                        worksheet.conditional_format(f'{col_letter}{row}', {
                            'type': 'formula',
                            'criteria': f'=AND({col_letter}{row}<MAX({row_cells}),{col_letter}{row}>=MAX({row_cells})*0.9)',
                            'format': workbook.add_format({'bg_color': '#C6EFCE'})  # Light green
                        })
                        
                        # Yellow for values within 75-90% of max
                        worksheet.conditional_format(f'{col_letter}{row}', {
                            'type': 'formula',
                            'criteria': f'=AND({col_letter}{row}<MAX({row_cells})*0.9,{col_letter}{row}>=MAX({row_cells})*0.75)',
                            'format': workbook.add_format({'bg_color': '#FFEB9C'})  # Light yellow
                        })
                        
                        # Light red for values below 75% of max
                        worksheet.conditional_format(f'{col_letter}{row}', {
                            'type': 'formula',
                            'criteria': f'={col_letter}{row}<MAX({row_cells})*0.75',
                            'format': workbook.add_format({'bg_color': '#FFC7CE'})  # Light red
                        })
                
                print(f"✓ Applied cross-model formatting to {len(accuracy_col_letters)} columns x {n_rows} rows")
                print("   Dark Green = Best model for that week")
                print("   Light Green = Within 90% of best")
                print("   Yellow = Within 75-90% of best")
                print("   Light Red = Below 75% of best")
                print("="*60 + "\n")
            
            else:
                print("⚠ Not enough accuracy columns for cross-model comparison\n")
